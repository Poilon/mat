...
const routes = Object.fromEntries(['config', 'auth', 'notebook', 'products', 'account'].map(name => [name, require('../api/' + name + '.js')]));
...
}).listen(Number(process.env.PORT) || 4175, '127.0.0.1', () => console.log('Nidelle: http://localhost:' + (process.env.PORT || 4175)));
