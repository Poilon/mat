...
(root => {
  ...
  function preferences(raw = {}) {
    ...
    return { mood:moods.some(m=>m.id===raw.mood)?raw.mood:'all', maxTime, servings, vegetarian:raw.vegetarian===true, avoid:(raw.avoid||'').trim(), dislikes:P.dislikes.filter(d=>raw.dislikes?.includes(d.id)).map(d=>d.id), pantry:P.pantry.filter(d=>raw.pantry?.includes(d.id)).map(d=>d.id) };
  }
  function suggestions(recipes, raw, excluded = [], random = Math.random) {
    const prefs=preferences(raw), terms=prefs.avoid.split(/[,;\n]+/).map(normalize).filter(Boolean);
    const pool=recipes.filter(r=>['lunch','dinner'].includes(r.type)&&r.time<=prefs.maxTime&&(!prefs.vegetarian||r.vegetarian)&&!P.dislikes.some(d=>prefs.dislikes.includes(d.id)&&r.ingredients.some(i=>d.pattern.test(normalize(i.name))))&&!terms.some(t=>r.ingredients.some(i=>normalize(i.name).includes(t))));
