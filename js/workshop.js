...
window.MietteWorkshop = (() => {
  ...
  function create(host) {
    let dietStamp, owner, state, feedback = '', memoryOnly = false, busy = false, job = 0;
    const key = value => 'miette-workshop-v1:' + value;
    function ensure() {
      const current = host.owner();
      if (owner === current && state) { refreshDiet(); return state; }
      owner = current; dietStamp=undefined; feedback = ''; memoryOnly = false; busy = false; job++;
      let raw = null;
      ...
      refreshDiet(); return state;
    }
    function refreshDiet() {
      const diet=window.PoumDiet.clean(host.notebook().diet),stamp=JSON.stringify(diet);
      if(stamp===dietStamp)return;
      const previous=JSON.stringify(state.options.diet||window.PoumDiet.clean());
      dietStamp=stamp; state.options.diet=diet;
      if(previous!==stamp&&state.entries.length){state.dirty=true;job++;busy=false;}
      persist();
    }
    function persist() {
    ...
    function render() {
      ...
      return `<div class="workshop-page"><div class="workshop-heading"><div><a class="btn-text" href="#plus">← L’atelier Plus</a><h1>Mes repas pendant la grossesse</h1><p>Choisissez vos préférences, puis ajustez les plats. Chaque recette contient ses précautions grossesse.</p></div><span class="workshop-demo">${host.billing().access.active ? 'Poum Plus actif' : 'Votre première semaine offerte'}</span></div><div id="workshop-access">${accessNote()}</div>${form()}<p id="workshop-feedback" class="workshop-feedback" role="status">${esc(feedback)}</p><div id="workshop-results">${results()}</div><p class="workshop-footnote"><span id="workshop-storage-note">${storageNote()}</span> Les propositions sont des idées de cuisine pour deux personnes, avec filtrage des exclusions déclarées, sans garantie d’absence d’allergènes ni adaptation à un régime médical. Les précautions et sources restent dans chaque recette.</p></div>`;
    }
    ...
    function options(form) {
      ...
      return { diet: window.PoumDiet.clean(host.notebook().diet), start: String(data.get('start')), maxTime: Number(data.get('maxTime')), vegetarian: data.has('vegetarian'), meals: data.get('meals'), mood: data.get('mood'), pantry: data.getAll('pantry'), dislikes: data.getAll('dislikes') };
    }
