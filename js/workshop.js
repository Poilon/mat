...
window.MietteWorkshop = (() => {
  ...
  function create(host) {
    ...
    function render() {
      ...
      return `<div class="workshop-page"><div class="workshop-heading"><div><a class="btn-text" href="#plus">← L’atelier Plus</a><h1>Préparer ma semaine</h1><p>Choisissez vos préférences, puis ajustez les repas proposés.</p></div><span class="workshop-demo">${host.billing().access.active ? 'Miamama Plus actif' : 'Votre première semaine offerte'}</span></div><div id="workshop-access">${accessNote()}</div>${form()}<p id="workshop-feedback" class="workshop-feedback" role="status">${esc(feedback)}</p><div id="workshop-results">${results()}</div><p class="workshop-footnote"><span id="workshop-storage-note">${storageNote()}</span> Les propositions sont des idées de cuisine pour deux personnes, sans prise en charge des allergies ni de vos besoins médicaux. Les précautions et sources restent dans chaque recette.</p></div>`;
    }
