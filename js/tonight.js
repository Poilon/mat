window.PoumTonight = (() => {
  ...
  function create(host) {
    ...
    function acceptMeal(m) {
      ...
      const title=document.querySelector('[data-dinner-heading]');if(title)title.textContent=V.labels[m.status]+'.';
      const home=document.querySelector('[data-tonight-home]');if(home)home.outerHTML=homeCard();
    }
    async function request(body) {
      ...
      const current=epoch;
      const result=await host.request(body);
    ...
    function homeCard() {
      ensure();const m=state.meal,r=m?.recipe;
      return `<aside class="tonight-home" data-tonight-home><div><span class="section-kicker">Une chose de moins à gérer</span><h2>${m?V.labels[m.status]+'.':'Ce soir, on te simplifie le dîner.'}</h2><p>${r?esc(r.title)+' · '+V.progress(m):'Trois idées selon vos envies, les courses prêtes et un proche qui peut prendre le relais.'}</p><a class="btn btn-primary" href="#cesoir">${m?'Retrouver mon dîner':'Trouver mon dîner'} <span aria-hidden="true">→</span></a>${m?'<small>'+new Date(m.createdAt).toLocaleDateString('fr-FR',{day:'numeric',month:'long'})+'</small>':'<small>Le premier dîner est offert, sans compte.</small>'}</div><img src="assets/brand/carnet-cuisine-v4-320.webp" alt="" width="320" height="213" loading="lazy"></aside>`;
    }
    ...
    function readForm() {const f=new FormData(document.querySelector('#tonight-form'));return T.preferences({...Object.fromEntries(f),vegetarian:f.has('vegetarian'),dislikes:f.getAll('dislikes'),pantry:f.getAll('pantry')});}
    function suggest(prefs,more=false) {return run(async()=>{const result=await request({action:'suggest',preferences:prefs,excluded:more?excluded:[]});state.preferences=result.preferences;choices=result.choices;remaining=result.remaining;excluded=[...(more?excluded:[]),...choices.map(c=>c.id)];pendingChoice=null;view='choices';save();});}
    ...
    function click(button) {
      ...
      else if(action==='tonight-choose')run(async()=>{if(!pendingChoice||pendingChoice.recipeId!==button.dataset.id)pendingChoice={requestId:crypto.randomUUID(),recipeId:button.dataset.id};const r=await request({action:'choose',...pendingChoice});state.meal=r.meal;state.trialUsed=true;selectedId=r.meal.id;shareURL='';view='meal';save();});
      ...
      else if(action==='tonight-cook'){patch('status','cooking',true);}
    ...
    async function patch(field,value,cook=false) {
      ...
      try{const result=await request({action:'patch',id:state.meal.id,field,value});acceptMeal(result.meal);if(cook){const details=document.querySelector('.dinner-method');if(details){details.open=true;details.scrollIntoView({block:'start'});}}}
    ...
    function statusClick(e){const b=e.target.closest('#tonight-page [data-dinner-status]');if(b&&!b.disabled){patch('status',b.dataset.dinnerStatus,b.dataset.dinnerStatus==='cooking');return true;}return false;}
    ...
    return {render,homeCard,click,submit,change,statusClick,enter,forget: value => {try{localStorage.removeItem('poum-tonight-v1:'+value);}catch{}}};
  }
  return {create};
})();
