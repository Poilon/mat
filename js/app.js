(() => {
  ...
  const Tonight = window.PoumTonight.create({
    owner: () => Cloud.storageKey, ready: () => !['checking', 'loading', 'unavailable'].includes(Cloud.state.status), page: () => route.page, vegetarian: () => store.vegetarian, diet: () => store.diet,
    request: body => Cloud.request('tonight', { method: 'POST', body: JSON.stringify(body) }), notify: toast,
    shopping: meal => {
      if(!window.PoumDiet.allows(meal.recipe,store.diet))throw Error('Ce dîner contient un ingrédient exclu dans votre profil. Choisissez un autre dîner.');
  ...
  const Journey = window.PoumJourney.create({
    ...
    openTask: id => {
      $('#detail-dialog').close();
      const focus = () => { const card=[...document.querySelectorAll('[data-task]')].find(el=>el.dataset.task===id);if(!card)return;card.querySelector('details').open=true;card.scrollIntoView({block:'center',behavior:'instant'});card.querySelector('[name=date]')?.focus({preventScroll:true}); };
      if(route.page==='calendrier'){rerender();focus();}else{window.addEventListener('hashchange',focus,{once:true});go('calendrier');}
    }
  });

  function readStore(provided) {
    const defaults = { journey: window.PoumPregnancy.empty(), diet: window.PoumDiet.clean(), name: '', vegetarian: false, favorites: [], products: [], menus: {}, shopping: [] };
    try {
      ...
      defaults.journey = window.PoumPregnancy.clean(raw.journey);
      defaults.vegetarian = raw.vegetarian === true; defaults.diet = window.PoumDiet.clean(raw.diet);
  ...
  function productBadge(f){const a=f.analysis;return `<span class="status-badge status-${escape(a.status)} off-badge">${icon(D.statuses[a.status]?.icon||'info',12)}${escape(a.label)}</span>`;}
  function allergenNames(tags){const names={milk:'Lait',eggs:'Œufs',gluten:'Gluten',wheat:'Blé',barley:'Orge',rye:'Seigle',oats:'Avoine',nuts:'Fruits à coque',almonds:'Amandes',hazelnuts:'Noisettes',peanuts:'Arachides',soybeans:'Soja',soy:'Soja',fish:'Poisson',crustaceans:'Crustacés',molluscs:'Mollusques',sesame:'Sésame','sesame-seeds':'Sésame',celery:'Céleri',mustard:'Moutarde',lupin:'Lupin','sulphur-dioxide-and-sulphites':'Sulfites'};return [...new Set((tags||[]).map(t=>{const key=t.replace(/^\w{2}:/,'');return names[key]||key.replace(/-/g,' ');} ))].join(', ');}
  function productFacts(f){
    const a=f.analysis,storage=f.conservation_conditions_fr||f.conservation_conditions;
    return `<section class="off-facts"><h3>Ce que la fiche indique</h3>${a.facts.length?`<ul class="off-mentions">${a.facts.map(fact=>`<li><strong>${escape(fact.title)}</strong><span>${fact.matches.map(m=>escape(m.label)+' : '+escape(m.term)).join(' · ')}</span></li>`).join('')}</ul><p class="off-fact-note">Une mention peut concerner un seul ingrédient. Elle décrit la fiche, pas un contrôle du produit.</p>`:''}<dl><div><dt>Composition</dt><dd>${f.ingredients_text_fr||f.ingredients_text?'Liste d’ingrédients disponible ci-dessous':f.ingredients.length?'Ingrédients structurés disponibles ; liste de l’emballage à comparer':'Non renseignée'}</dd></div><div><dt>Allergènes déclarés</dt><dd>${escape(allergenNames(f.allergens_tags)||'Non renseignés')}</dd></div><div><dt>Traces éventuelles déclarées</dt><dd>${escape(allergenNames(f.traces_tags)||'Non renseignées')}</dd></div>${storage?`<div><dt>Conservation indiquée</dt><dd>${escape(storage)}</dd></div>`:''}</dl><p class="off-fact-note">Une information non renseignée ne signifie pas une absence d’allergène ou de traces. Comparez avec l’emballage.</p>${f.last_modified_t?`<p class="off-fact-note">Fiche OFF modifiée le ${new Date(f.last_modified_t*1000).toLocaleDateString('fr-FR')} · cette date ne vérifie pas votre lot.</p>`:''}</section>`;
  }
  ...
  function foodCard(f) {
    return `<article class="food-card" data-category="${escape(f.category)}">
      ...
      <button class="food-card-open" data-action="food" data-id="${escape(f.id)}" aria-label="Voir les précautions : ${escape(f.name)}">
        ...
        <div class="food-card-content"><h3>${escape(f.name)}</h3>${f.origin==='off'?productBadge(f):badge(f.status)}${f.origin==='off'?`<p class="off-card-summary">${escape(f.summary)}</p>${f.analysis.facts.length?`<p class="off-card-facts">${f.analysis.facts.map(fact=>escape(fact.title)).join(' · ')}</p>`:''}<p class="off-card-brand">${escape([f.brands,f.quantity].filter(Boolean).join(' · '))}</p>`:`<p>${escape(f.summary)}</p>`}</div>
      </button>
    </article>`;
  }
  function recipeCard(r) {
    return `<article class="recipe-card">${favoriteButton('recipe', r.id)}<button class="recipe-card-open" data-action="recipe" data-id="${r.id}">
      ...
      <div class="recipe-card-content"><h3>${r.title}</h3>${route.page === 'recettes' ? `<p class="recipe-intro">${r.subtitle}</p>` : ''}<div class="recipe-tags"><span>${icon(r.vegetarian ? 'leaf' : 'recipe', 12)}${r.tags[0]}</span><span>${r.tags[1]}</span></div>${window.PoumDiet.clean(store.diet).completed?'<span class="j-meal-confirmation">Selon vos exclusions renseignées</span>':''}<p class="recipe-pregnancy-note">${icon('book', 13)}Précautions grossesse dans la fiche</p></div>
    </button></article>`;
  }
  ...
  function sidebar() {
    const links = [['accueil', 'home', 'Aujourd’hui'], ['calendrier', 'calendar', 'Mon calendrier'], ['examens', 'book', 'Mes examens'], ['cesoir', 'recipe', 'Ce soir'], ['envies', 'leaf', 'Mes envies'], ['aliments', 'search', 'Les aliments'], ['recettes', 'recipe', 'Les recettes']];
    ...
    return `<button class="icon-button sidebar-close" data-action="close-menu" aria-label="Fermer le menu">${icon('close')}</button>
      <a class="brand" href="#accueil" aria-label="Poum, accueil"><img src="assets/brand/mark.svg" alt="" width="32" height="32"><span class="wordmark">poum</span></a><p class="brand-tagline">Calendrier, examens et repas.</p>
  ...
  function recipeSettings() {
    ...
    return {
      ...
      cuisine: route.params.get('cuisine') || 'all',
  ...
  function recipeMatches(recipe, query) {
    const text = R.normalize(recipe.title + ' ' + (recipe.cuisine || '') + ' ' + recipe.ingredients.map(i => i.name).join(' '));
    return query.split(/\s+/).filter(Boolean).every(word => text.includes(word));
  }
  function filteredRecipes() {
    const s = recipeSettings();
    return D.recipes.filter(r => window.PoumDiet.allows(r,store.diet) && (s.cuisine === 'all' || r.cuisine === s.cuisine) && (s.access === 'all' || (s.access === 'plus') === r.premium) && (!s.vegetarian || r.vegetarian) && (s.type === 'all' || r.type === s.type) && (s.collection === 'all' || r.collection === s.collection) && (!s.duration || r.time <= s.duration) && recipeMatches(r, s.q))
  ...
  function recipePage() {
    ...
    return `<div class="recipes-page"><section class="recipe-welcome" aria-labelledby="recipe-welcome-title"><div><span class="section-kicker">Cuisine & grossesse</span><h1 id="recipe-welcome-title">Enceinte, on mange quoi&nbsp;?</h1><p>Un plat, un ingrédient, une idée pour ce soir. Les précautions grossesse accompagnent chaque recette.</p><span class="recipe-welcome-note">${D.freeRecipeCount} recettes gratuites · ${D.recipes.length - D.freeRecipeCount} avec Plus</span></div><img class="recipe-sketch" src="assets/brand/carnet-cuisine-v4-640.webp" srcset="assets/brand/carnet-cuisine-v4-320.webp 320w, assets/brand/carnet-cuisine-v4-640.webp 640w" sizes="(max-width: 600px) 75px, 250px" alt="" width="1536" height="1024" loading="lazy"></section>
      ...
      <div class="recipe-access-filters" role="group" aria-label="Accès aux recettes">${[['all', 'Toutes'], ['free', '20 gratuites'], ['plus', (D.recipes.length-D.freeRecipeCount)+' Plus']].map(([id, label]) => `<button class="category-chip ${s.access === id ? 'active' : ''}" data-action="recipe-access-filter" data-access="${id}" aria-pressed="${s.access === id}">${label}</button>`).join('')}</div><form id="recipe-search" class="search-box" role="search">${icon('search', 20)}<label class="sr-only" for="recipe-query">Rechercher une recette ou un ingrédient</label><input id="recipe-query" name="q" type="search" maxlength="100" value="${escape(route.params.get('q') || '')}" placeholder="Un ingrédient, un plat… chocolat, tacos, courgette"><button class="btn btn-primary" type="submit">Rechercher ${icon('arrow', 15)}</button></form>
      ...
      <div class="recipe-toolbar"><div class="recipe-select"><label for="recipe-cuisine">Cuisine du monde</label><select id="recipe-cuisine"><option value="all">Toutes les cuisines</option>${[...new Set(D.recipes.map(r=>r.cuisine).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'fr')).map(c=>`<option value="${escape(c)}" ${s.cuisine===c?'selected':''}>${escape(c)}</option>`).join('')}</select></div><button class="category-chip ${s.vegetarian ? 'active' : ''}" data-action="vegetarian" aria-pressed="${s.vegetarian}">${icon('leaf', 15)}Végétarien</button><div class="recipe-select"><label for="recipe-duration">${icon('clock', 14)}Temps total</label><select id="recipe-duration"><option value="0">J’ai le temps</option>${[20, 30, 45].map(n => `<option value="${n}" ${s.duration === n ? 'selected' : ''}>${n} min maximum</option>`).join('')}</select></div><div class="recipe-select"><label for="recipe-sort">Trier par</label><select id="recipe-sort"><option value="inspiration">Nos inspirations</option><option value="rapide" ${s.sort === 'rapide' ? 'selected' : ''}>Les plus rapides</option><option value="az" ${s.sort === 'az' ? 'selected' : ''}>Nom, de A à Z</option></select></div></div>
      <div id="recipe-active-filters">${recipeActiveFilters()}</div><div id="recipe-results">${recipeResults()}</div><p class="photo-caption">1 000 recettes et déclinaisons maison, non testées en cuisine. Les plats du monde sont des adaptations signalées dans les fiches. Photos d’inspiration ; suivez les ingrédients écrits.</p></div>`;
  }
  function recipeActiveFilters() {
    const s = recipeSettings();
    const collection = D.recipeCollections.find(c => c.id === s.collection);
    const labels = [s.cuisine !== 'all' && s.cuisine, collection?.label, s.type !== 'all' && recipeTypes[s.type], s.duration && `${s.duration} min maximum`, s.vegetarian && 'Végétarien', s.q && `« ${route.params.get('q')} »`].filter(Boolean);
  ...
  function guidePage() {
    ...
    return `${heading('Les précautions en cuisine', 'Lavage, cuisson, conservation : les gestes à connaître pendant la grossesse.')}<div class="guide-grid">${cards.map(([symbol, title, text, source]) => `<article class="guide-card"><div class="tip-icon">${icon(symbol, 21)}</div><h2>${title}</h2><p>${text}</p>${sourceLink(source)}</article>`).join('')}</div><section class="faq-section"><h2>Les questions qui reviennent.</h2>${[
      ...
      ['Comment lire les produits Open Food Facts ?', 'Poum distingue les signaux à éviter ou à limiter, les précautions de préparation et les fiches incomplètes. « Aucun signal repéré » décrit une détection partielle, pas une garantie de compatibilité. Les mentions, ingrédients, allergènes et traces disponibles sont affichés séparément.'],
  ...
  function accountPanel() {
    ...
    const labels = { grossesse: 'Ma grossesse', calendrier: 'Mon calendrier', examens: 'Mes examens', envies: 'Mes envies du moment', login: 'Se connecter', signup: 'Créer mon compte', forgot: 'Recevoir le lien de récupération', reset: 'Enregistrer le nouveau mot de passe' };
    return `<section class="account-panel">${intro}<p>Retrouvez vos favoris, vos menus et votre liste de courses sur votre téléphone et votre ordinateur.</p><div class="account-tabs" role="group" aria-label="Connexion ou inscription"><button class="btn ${mode === 'login' ? 'btn-primary' : 'btn-outline'}" data-action="auth-mode" data-mode="login">Se connecter</button><button class="btn ${mode === 'signup' ? 'btn-primary' : 'btn-outline'}" data-action="auth-mode" data-mode="signup">Créer un compte</button></div><form id="auth-form" data-mode="${mode}">
      ...
      ${mode === 'signup' ? '<p class="small">Après la création du compte, indiquez vos allergies et les aliments que vous n’aimez pas pour personnaliser les menus.</p>' : ''}
      ${mode === 'login' || mode === 'signup' ? '<label class="check-label account-import"><input name="import" type="checkbox"> Ajouter le carnet de cet appareil (y compris mon suivi de grossesse) à mon compte</label>' : ''}
  ...
  function dietSummary() { return Journey.chips(); }
  function dietFields() {
  ...
  function renderRoute(keepDialog = false) {
    ...
    const pages = { grossesse: Journey.setup, calendrier: Journey.calendar, examens: Journey.exams, envies: Journey.cravings, bienvenue: Onboarding.render, cesoir: Tonight.render, accueil: homePage, aliments: explorePage, recettes: recipePage, favoris: favoritesPage, menus: plannerPage, courses: shoppingPage, guide: guidePage, sources: sourcesPage, profil: profilePage, confidentialite: privacyPage, plus: plusPage, atelier: Workshop.render };
    $('#main').innerHTML = `<div class="page-content">${pages[route.page]()}</div>${footer()}`;
    document.querySelector('#poum-bottom-nav')?.remove();
    const bottom=document.createElement('nav');bottom.id='poum-bottom-nav';bottom.className='j-bottom-nav';bottom.setAttribute('aria-label','Raccourcis du quotidien');
  ...
  function replaceNotebook(notebook, { preserveEdits = false } = {}) {
    ...
    Journey.ensure();
    if(preserveEdits && Journey.editing())return;
  ...
  function foodDetail(id) {
    ...
    const top = `<div class="dialog-food-top"><div class="dialog-food-art">${isOFF && f.image_front_small_url ? `<img src="${escape(f.image_front_small_url)}" alt="${escape(f.name)}" referrerpolicy="no-referrer">` : art(f.art)}</div><div class="dialog-food-heading"><span class="eyebrow">${isOFF ? 'PRODUIT OPEN FOOD FACTS' : 'LE GUIDE POUM'}</span><h2 id="dialog-title">${escape(f.name)}</h2>${isOFF?productBadge(f):badge(f.status)}<p>${escape(isOFF ? [f.brands, f.quantity].filter(Boolean).join(' · ') : f.summary)}</p></div></div>`;
    let body;
    if (isOFF) {
      const a = f.analysis;
      ...
      const ingredientText=f.ingredients_text_fr||f.ingredients_text||f.ingredients.map(i=>typeof i?.text==='string'?i.text:typeof i?.id==='string'?i.id.replace(/^\w{2}:/,''):'').filter(Boolean).join(', ')||'La liste des ingrédients n’est pas renseignée. Consultez l’emballage.';
      body = `<div class="advice-box ${f.status} off-answer"><p><strong>${escape(a.label)}</strong></p><p>${escape(a.summary)}</p></div>${a.missing.length?`<p class="missing-data">${icon('info',14)}${a.missing.map(escape).join(' · ')}</p>`:''}${a.qualityIssues?'<p class="missing-data">Une incohérence est signalée par Open Food Facts. Vérifiez les données avec l’emballage.</p>':''}${productFacts(f)}${a.flags.length?`<h3>Les précautions qui concernent cette fiche</h3>${a.flags.map(productFlag).join('')}`:''}<h3>Les ingrédients renseignés</h3><p class="ingredient-text">${escape(ingredientText)}</p>${nutrition.length ? `<h3>Valeurs déclarées pour 100 g / 100 ml</h3><div class="nutrition-grid">${nutrition.map(([k, label, unit]) => `<div class="nutrition-item"><b>${number(f.nutriments[k])} ${unit}</b><span>${label}</span></div>`).join('')}</div><p class="dialog-disclaimer">Ces valeurs nutritionnelles ne déterminent pas la sécurité du produit pendant la grossesse.</p>` : ''}<details class="product-unknowns"><summary>Ce que cette analyse ne peut pas confirmer</summary><p>Poum recherche certains signaux dans les données collaboratives. Aucun produit n’est certifié compatible avec la grossesse, même sans signal détecté.</p><ul>${a.unknowns.map(t=>`<li>${escape(t)}</li>`).join('')}</ul><p>Les termes cités sont normalisés pour la détection ; ils ne remplacent pas l’étiquette.</p></details>${a.sources.length?sourceFooter(a.sources):''}<div class="sources-inline"><p>CODE-BARRES : ${escape(f.code)} · DONNÉES COLLABORATIVES ODBL</p><a class="source-link" href="https://world.openfoodfacts.org/product/${encodeURIComponent(f.code)}" target="_blank" rel="noopener noreferrer">Voir la fiche originale Open Food Facts ${icon('external', 13)}</a></div>`;
    } else {
  ...
  function recipePreview(r, message, loading = false) {
    openDialog(`<img class="dialog-recipe-photo" src="assets/${r.image}.jpg" alt="Photo d’inspiration culinaire"><div class="dialog-content"><span class="eyebrow">Recette Plus · ${r.time} minutes</span><h2 id="dialog-title">${escape(r.title)}</h2><section class="recipe-paywall"><h3>${loading ? 'Ouverture de la préparation…' : 'Cette recette fait partie de Plus'}</h3><p role="status">${escape(message)}</p>${loading ? '' : `<p>Les 980 recettes Plus, les menus automatiques et leurs courses : <b>4,90 €/mois</b> ou <b>29,90 € pour 9 mois</b>.</p><button class="btn btn-primary" data-action="plus-offer">Découvrir l’accès Plus</button><button class="btn-text" data-action="recipe" data-id="${r.id}">Déjà abonnée ? Réessayer l’ouverture</button><a class="btn-text" href="${window.MietteRuntime?.cloud === false ? escape(window.MietteRuntime.appURL) : ''}#profil">Me connecter à mon compte</a>`}</section><h3>Les ingrédients</h3><ul class="ingredients-list">${r.ingredients.map(i => `<li>${escape(i.name)}<b>${number(i.quantity)} ${escape(i.unit)}</b></li>`).join('')}</ul><p class="allergens"><strong>Allergènes :</strong> ${escape(r.allergens)}</p><div class="advice-box"><strong>Pendant la grossesse</strong><p>${escape(r.safety)}</p></div>${!window.PoumDiet.allows(r,store.diet)?'<p class="preference-note" role="alert">Cette recette contient un ingrédient exclu dans votre profil.</p>':''}${window.PoumCooking.storage(r)}${sourceFooter(r.sources || ['spf', 'toxo'])}<p class="small muted">Les ingrédients, allergènes, précautions et sources restent accessibles gratuitement. Plus donne accès aux étapes de préparation.</p></div>`, { type: loading ? 'recipe-loading' : 'recipe-preview', id: r.id });
  }
  ...
  function pickRecipe(date, meal) {
    const recipes = D.recipes.filter(r => (!store.vegetarian || r.vegetarian) && window.PoumDiet.allows(r,store.diet));
  ...
  function assignRecipe(id, date, meal) {
    const candidate=recipesById.get(id); if(candidate&&!window.PoumDiet.allows(candidate,store.diet)){toast('Cette recette contient un ingrédient exclu dans votre profil.','info');return;}
  ...
  function addIngredients(recipes) {
    if(recipes.some(({recipe})=>!window.PoumDiet.allows(recipe,store.diet))){toast('Un plat contient un ingrédient exclu. Remplacez-le avant de préparer les courses.','info');return false;}
    let count = 0;
    ...
    persist(); toast(`${count} ingrédients ajoutés à votre liste de courses.`, 'bag'); return true;
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (Journey.click(trigger) || Onboarding.click(trigger) || Tonight.click(trigger) || Workshop.click(trigger)) return;
    switch (action) {
      ...
      case 'confirm-reset': store = { diet: window.PoumDiet.clean(), name: '', vegetarian: false, favorites: [], products: [], menus: {}, shopping: [] }; Workshop.clear(); RecipeAccess.clear(); productMap.clear(); persist(); API.clearCache(); apiState = { key: '', products: [], loading: false, count: 0 }; $('#detail-dialog').close(); rerender(); toast('Votre carnet a été effacé de ce navigateur.', 'check'); break;
      case 'diet-later': resumeCommerce().catch(error=>toast(error.message,'info')); break;
  ...
  document.addEventListener('submit', event => {
    ...
    if (!known.includes(form.id) && !form.matches('[data-journey-form]')) return;
    event.preventDefault();
    if (Journey.submit(form) || Onboarding.submit(form)) return;
    ...
    if (form.id === 'home-search' || form.id === 'explore-search') {
      ...
      let diet; try { diet = window.PoumDiet.validate({allergies:data.getAll('allergy'),otherAllergies:String(data.get('otherAllergies')||''),avoid:store.diet.avoid,consent:data.has('diet-consent'),completed:true,temporary:store.diet.temporary}); } catch(error) {
  ...
  document.addEventListener('input', event => {
    if (Journey.input(event) || Onboarding.input(event)) return;
  ...
  document.addEventListener('change', event => {
    if (Journey.change(event) || Tonight.change(event)) return;
    ...
    if (event.target.id === 'recipe-cuisine') updateRecipeFilters({ cuisine: event.target.value });
  ...
  function updateOnline() {
    ...
    el.innerHTML = navigator.onLine ? '' : `<div class="offline-banner">${icon('offline', 14)}Vous êtes hors connexion. Le guide et votre carnet restent accessibles sur cet appareil.</div>`;
  }
  ...
  window.addEventListener('miamama:billing', () => {
    ...
    if (['plus', 'atelier', 'profil', 'calendrier', 'examens', 'grossesse', 'envies', 'accueil'].includes(route.page) && !authBusy) replaceNotebook(store, { preserveEdits: true });
  });
