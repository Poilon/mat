(() => {
  ...
  function readStore(provided) {
    const defaults = { diet: window.PoumDiet.clean(), name: '', vegetarian: false, favorites: [], products: [], menus: {}, shopping: [] };
    try {
  ...
  function homePage() {
    const featuredRecipes = recipeHighlights.map(id => recipesById.get(id)).filter(r => (!store.vegetarian || r.vegetarian) && window.PoumDiet.allows(r,store.diet)).slice(0, 3);
  ...
  function dietSummary() {
    const d=window.PoumDiet.clean(store.diet),n=d.allergies.length+Boolean(d.otherAllergies)+Boolean(d.avoid);
    return n?`<p class="preference-note">Vos exclusions sont appliquées. Les recettes avec un allergène déclaré ou possible sont écartées. Vérifiez les étiquettes et les traces. <a href="#profil">Modifier mes préférences</a></p>`:'';
  }
  function dietFields() {
