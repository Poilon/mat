(() => {
  ...
  function sidebar() {
    ...
    return `<button class="icon-button sidebar-close" data-action="close-menu" aria-label="Fermer le menu">${icon('close')}</button>
      <a class="brand" href="#accueil" aria-label="Miamama, accueil"><img src="assets/brand/mark.svg" alt="" width="32" height="32"><span class="wordmark">miam<span>ama</span></span></a><p class="brand-tagline">Le guide pour manger enceinte.</p>
  ...
  function topbar() {
    return `<button class="mobile-menu" data-action="menu" aria-label="Ouvrir le menu" aria-expanded="false" aria-controls="sidebar">${icon('menu', 22)}</button><div class="breadcrumb"><a class="brand-home" href="#accueil"><img src="assets/brand/mark.svg" alt="" width="32" height="32"><span>Miamama<small>Alimentation & grossesse</small></span></a><span aria-hidden="true">/</span><b>${labels[route.page]}</b></div><div class="topbar-right"><a href="#plus" class="topbar-plus">${Billing.active ? 'Mon accès Plus' : 'Plus'}</a><a href="#profil" class="profile-trigger" aria-label="Mon compte et mes préférences"><span class="avatar">${store.name ? escape(store.name.charAt(0).toUpperCase()) : icon('user', 17)}</span><span>${escape(store.name || (Cloud.state.user ? 'Mon compte' : 'Se connecter'))}</span></a></div>`;
  }
  function footer() {
  ...
  function homePage() {
    ...
    return `<div class="home-dateline"><span>${store.name ? `Bonjour ${escape(store.name)}.` : 'Bonjour.'}</span><time datetime="${localDate()}">${date}</time></div>
      <section class="home-editorial" aria-labelledby="home-title"><div class="home-search-panel"><div class="pregnancy-signature"><img src="assets/brand/mark.svg" alt="" width="46" height="46"><span class="section-kicker">À table pendant<br>la grossesse</span></div><h1 id="home-title"><em>Enceinte,</em><br>je peux en manger&nbsp;?</h1><p>Du chèvre, des sushis, un café… Ce que vous pouvez manger pendant la grossesse, les précautions et les façons de cuisiner.</p>${searchForm('home-search')}<div class="suggestions"><span>Par exemple</span>${['Patate douce', 'Ananas', 'Mozzarella', 'Café'].map(q => `<a class="suggestion" href="${href('aliments', { q })}">${q}</a>`).join('')}</div><a class="home-guide-link" href="#aliments">${D.foods.length} aliments expliqués pour la grossesse ${icon('arrow', 16)}</a><p class="home-source-note">Compatible, à limiter ou à éviter : chaque fiche explique pourquoi, dans quelles conditions et avec quelles sources. <a href="#sources">Comment lire les repères</a></p></div>
      <figure class="tonight-feature"><button data-action="recipe" data-id="${tonight.id}" aria-label="Voir la recette : ${escape(tonight.title)}"><img class="home-food-photo" src="assets/${tonight.image}.jpg" alt="Un plat de lasagnes gratinées" fetchpriority="high" width="760" height="530"><span class="tonight-copy"><span class="section-kicker">Cuisiner pendant la grossesse</span><strong>${escape(tonight.title)}</strong><span>${tonight.time} min · Pour 2 personnes ${icon('arrow', 17)}</span></span></button><figcaption>Cuisson, ingrédients, précautions grossesse : tout est dans la fiche. Photo d’inspiration.</figcaption></figure></section>
  ...
  function plusPage() {
    ...
    return `<div class="offer-page">${billingFeedback()}${Billing.state.canManage ? memberPanel() : ''}
      <section class="offer-hero"><div class="offer-hero-copy">${plusStamp()}<h1>Les repas de la semaine,<br>pendant la grossesse.</h1><p>Quand les goûts changent ou que cuisiner prend trop de temps, l’atelier propose des repas avec les ingrédients que vous aimez et ceux qu’il reste dans le placard.</p><p>Vous pouvez changer un plat et emporter les courses avec les recettes et leurs précautions grossesse.</p><button class="btn btn-primary offer-primary" data-action="plus-preview">Essayer ma semaine offerte ${icon('arrow', 17)}</button><small>Avec un compte gratuit, sans carte bancaire.</small><button class="btn-text offer-example-button" data-action="plus-example">D’abord, voir un exemple ${icon('arrow', 14)}</button></div>
