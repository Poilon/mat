(() => {
  ...
  function recipePage() {
    ...
    return `<div class="recipes-page"><section class="recipe-welcome" aria-labelledby="recipe-welcome-title"><div><span class="section-kicker">Cuisine & grossesse</span><h1 id="recipe-welcome-title">Enceinte, on mange quoi&nbsp;?</h1><p>${D.recipes.length} idées de repas avec les précautions à suivre pendant la grossesse. Cherchez un plat, un ingrédient ou une recette rapide.</p><span class="recipe-welcome-note">${D.freeRecipeCount} recettes complètes gratuites · ${D.recipes.length - D.freeRecipeCount} recettes avec Plus. Les précautions restent accessibles à toutes.</span></div><button class="recipe-welcome-feature" data-action="recipe" data-id="${featured.id}"><img src="assets/${featured.image}.jpg" alt="Lasagnes gratinées" width="760" height="530"><span>Les lasagnes du week-end ${icon('arrow', 16)}</span></button></section>
      ...
      <div class="recipe-access-filters" role="group" aria-label="Accès aux recettes">${[['all', 'Toutes les recettes'], ['free', 'Les 20 gratuites'], ['plus', 'Les 80 recettes Plus']].map(([id, label]) => `<button class="category-chip ${s.access === id ? 'active' : ''}" data-action="recipe-access-filter" data-access="${id}" aria-pressed="${s.access === id}">${label}</button>`).join('')}</div><form id="recipe-search" class="search-box" role="search">${icon('search', 20)}<label class="sr-only" for="recipe-query">Rechercher une recette ou un ingrédient</label><input id="recipe-query" name="q" type="search" maxlength="100" value="${escape(route.params.get('q') || '')}" placeholder="Un ingrédient, un plat… chocolat, tacos, courgette"><button class="btn btn-primary" type="submit">Rechercher ${icon('arrow', 15)}</button></form>
  ...
  function premiumNudge(place = 'home') {
    const copy = {
      home: ['Et les six autres dîners ?', 'Les 100 recettes, une semaine selon vos goûts et les courses regroupées. C’est ce que comprend Plus.'],
    ...
    return `<aside class="premium-nudge premium-nudge-${place}" data-premium-place="${place}"><div>${plusStamp()}<h2>${copy[0]}</h2><p>${copy[1]}</p><span class="premium-nudge-caption">${Billing.active ? 'Votre atelier Plus est ouvert' : 'Première semaine offerte · Plus : 4,90 €/mois ou 29,90 € les 9 mois'}</span></div><div class="premium-actions">${Billing.active ? '<a class="btn btn-primary" href="#atelier">Ouvrir mon atelier</a>' : `<button class="btn btn-primary" data-action="plus-offer">Découvrir les recettes & menus Plus ${icon('arrow', 16)}</button><button class="btn-text" data-action="plus-example">Voir une semaine en exemple</button>`}</div></aside>`;
  }
