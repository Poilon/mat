(() => {
  ...
  const Workshop = window.MietteWorkshop.create({
    ...
    generate: payload => {
      ...
      return Cloud.request('workshop', { method: 'POST', body: JSON.stringify(payload) }).then(result => { Billing.refresh(); return result; });
    }
  });
  ...
  function foodExplanation(f) {
    ...
    return `<section class="food-explanation" aria-labelledby="why-title"><h3 id="why-title">Pourquoi ce repère ?</h3><div class="advice-box ${f.status}"><p>${escape(e.why)}</p></div>${citations(e.sources)}<div class="evidence-mechanism"><h3>${escape(p.title)}</h3><p>${escape(p.mechanism)}</p>${citations(p.sources)}</div><div class="evidence-condition"><h3>Ce qui change la réponse</h3><p>${escape(p.condition)}</p>${e.note ? `<p class="evidence-note"><strong>Le repère en pratique.</strong> ${escape(e.note)}</p>` : ''}</div><p class="evidence-basis">${e.basis === 'specific' ? 'Synthèse Miamama : cet aliment, ce type de produit ou cette préparation est cité dans les références.' : 'Application par Miamama des recommandations générales à cette famille d’aliments ; les sources ne citent pas nécessairement cet aliment individuellement.'}</p></section>`;
  }
  ...
  function sidebar() {
    ...
    const notebook = [['favoris', 'heart', 'Mes favoris'], ['menus', 'calendar', 'Mes menus'], ['courses', 'bag', 'Ma liste de courses'], ['atelier', 'sparkle', 'Mon atelier Plus']];
    ...
    return `<button class="icon-button sidebar-close" data-action="close-menu" aria-label="Fermer le menu">${icon('close')}</button>
      <a class="brand" href="#accueil" aria-label="Miamama, accueil"><img src="assets/brand/mark.svg" alt="" width="37" height="37"><span class="wordmark">miam<span>ama</span></span></a><p class="brand-tagline">La grossesse a bon goût..</p>
      <nav aria-label="Navigation principale"><p class="nav-label">AU QUOTIDIEN</p>${links.map(nav).join('')}<p class="nav-label secondary">MON PETIT CARNET</p>${notebook.map(nav).join('')}<p class="nav-label secondary">POUR M’ACCOMPAGNER</p>${nav(['guide', 'book', 'Les bons repères'])}${nav(['plus', 'sparkle', Billing.active ? 'Mon abonnement Plus' : 'Découvrir Miamama Plus'])}</nav>
  ...
  function topbar() {
    return `<button class="mobile-menu" data-action="menu" aria-label="Ouvrir le menu" aria-expanded="false" aria-controls="sidebar">${icon('menu', 22)}</button><div class="breadcrumb">${icon('leaf', 16)}<span>Miamama</span>${icon('chevron', 11)}<b>${labels[route.page]}</b></div><div class="topbar-right"><a href="#plus" class="topbar-plus">${icon('sparkle', 13)}${Billing.active ? 'Mon accès Plus' : 'Miamama Plus'}</a><span class="topbar-note">${icon('heart', 13)}À vos côtés pendant la grossesse</span><a href="#profil" class="profile-trigger" aria-label="Mon compte et mes préférences"><span class="avatar">${escape(store.name ? store.name.charAt(0).toUpperCase() : 'M')}</span><span>${escape(store.name || (Cloud.state.user ? 'Mon compte' : 'Se connecter'))}</span>${icon('down', 13)}</a></div>`;
  }
  function footer() {
    return `<footer class="footer"><span><span class="footer-brand">miamama</span> &nbsp; La grossesse a bon goût. <span class="footer-heart">♡</span></span><div class="footer-links"><a href="#plus">Miamama Plus</a><a href="#sources">Sources & méthode</a><a href="#confidentialite">Vos données</a><span>© 2026 Miamama</span></div></footer>`;
  }
  function migrationNote() {
    const runtime = window.MietteRuntime;
    if (runtime?.cloud !== false || !runtime.appURL) return '';
    return `<aside class="migration-note"><div><b>Miamama a une nouvelle maison.</b><p>Compte, carnet synchronisé et scan photo : retrouvez la version connectée.</p></div><div class="dialog-actions"><a class="btn btn-primary" href="${escape(runtime.appURL)}">Ouvrir l’application ${icon('arrow', 15)}</a><button class="btn btn-outline" data-action="transfer-notebook">Transférer mon carnet</button></div></aside>`;
  }
  ...
  function homePage() {
    ...
    return `<div class="page-heading"><div><h1>${store.name ? `Bonjour ${escape(store.name)},` : 'Bonjour, vous.'} ${icon('sun', 23)}</h1><p>Une nouvelle journée pour prendre soin de vous, et de bébé.</p></div><span class="heading-pill">${icon('leaf', 13)} À votre rythme, à chaque trimestre</span></div>
      ...
      ${premiumNudge('home')}<div class="home-footer-note">${icon('heart', 25)}<p>Chaque grossesse est unique. Miamama vous donne des repères généraux ; votre sage-femme ou médecin vous accompagne personnellement. <a href="#sources">Comprendre nos conseils</a></p></div>`;
  }
  ...
  function billingNote() {
    ...
    if (b.loaded && !b.configured && b.mode !== 'mirror') return '<p class="plus-launch-note">L’atelier reste offert pendant l’ouverture des paiements. Aucun abonnement ne commence sans achat.</p>';
    return '<p class="plus-launch-note">Le paiement s’effectue sur Stripe. Retrouvez vos factures et la gestion du renouvellement dans votre compte Miamama.</p>';
  }
  ...
  function memberPanel() {
    ...
    return `<section class="member-panel"><div class="member-symbol">${icon('sparkle', 28)}</div><div><span class="eyebrow">${active ? 'VOTRE QUOTIDIEN AVEC PLUS' : 'VOTRE ABONNEMENT'}</span><h2>${active ? 'Bienvenue dans votre atelier.' : 'Retrouvez votre accès Plus.'}</h2><p>${active ? b.access.plan === 'pass' ? `Votre pass est actif jusqu’au ${until}, sans renouvellement automatique.` : b.access.cancelAtPeriodEnd ? `Le renouvellement est arrêté. Plus reste accessible jusqu’au ${until}.` : `Votre abonnement est actif. Période payée jusqu’au ${until}.` : b.access.subscriptionStatus === 'past_due' ? 'Le paiement doit être mis à jour pour retrouver les nouvelles semaines.' : 'Vos menus enregistrés restent accessibles. Vous pouvez gérer vos achats depuis Stripe.'}</p><div class="dialog-actions">${active ? '<a class="btn btn-primary" href="#atelier">Préparer ma prochaine semaine</a>' : ''}${b.canManage ? '<button class="btn btn-outline" data-action="billing-portal">Mon abonnement & mes factures</button><button class="btn-text" data-action="billing-restore">Actualiser mon accès</button>' : ''}</div></div></section>`;
  }
  function premiumNudge(place = 'home') {
    const copy = {
      home: ['Les envies changent. Le menu suit.', 'Vos goûts, votre placard, votre temps : une semaine à cuisiner, avec les courses déjà rassemblées.'],
      recipes: ['Une recette qui plaît. Une semaine qui donne envie.', 'Laissez l’atelier composer les prochains repas, puis gardez vos coups de cœur.'],
      favorites: ['Vos prochains dîners méritent aussi un coup de cœur.', 'Composez une semaine, changez un plat et gardez vos idées préférées.'],
      shopping: ['Le menu d’abord. Les courses suivent.', 'Les ingrédients de toute la semaine sont regroupés, avec les quantités à prévoir.'],
      recipe: ['Et si toute la semaine donnait envie ?', 'Retrouvez des plats selon vos goûts, les courses et le carnet complet à partager.']
    }[place];
    return `<aside class="premium-nudge premium-nudge-${place}" data-premium-place="${place}"><div class="premium-nudge-photos" aria-hidden="true"><img src="assets/recipes/gnocchi.jpg" alt="" width="160" height="160" loading="lazy"><img src="assets/recipes/curry.jpg" alt="" width="160" height="160" loading="lazy"></div><div>${plusStamp()}<h2>${copy[0]}</h2><p>${copy[1]}</p><span class="premium-nudge-caption">${Billing.active ? 'Votre atelier Plus est ouvert' : 'Première semaine offerte · Puis 4,90 €/mois ou le pass 9 mois'}</span></div><a class="btn ${place === 'home' ? 'btn-primary' : 'btn-outline'}" href="${Billing.active ? '#atelier' : '#plus'}">${Billing.active ? 'Ouvrir mon atelier' : 'Découvrir Miamama Plus'} ${icon('arrow', 16)}</a></aside>`;
  }
  ...
  function plusOffer() {
    ...
    openDialog(`<div class="dialog-content plus-dialog offer-dialog">${plusStamp()}<h2 id="dialog-title">Une semaine de moins<br>à organiser.</h2><p class="plus-lead">Vos envies du moment, les ingrédients à utiliser, les repas à préparer. L’atelier relie tout pour vous aider au quotidien pendant la grossesse.</p>${stats.ready ? `<div class="offer-proof"><b>Vous venez de préparer…</b><div><span><strong>${stats.meals}</strong> repas</span><span><strong>${stats.matches}</strong> avec vos ingrédients</span><span><strong>${stats.items}</strong> articles à prévoir</span></div></div>` : '<div class="offer-dialog-photo"><img src="assets/recipes/gnocchi.jpg" alt="Gnocchis dorés, une des recettes à composer dans l’atelier." width="600" height="220"><span>Vos goûts. Vos plats. Vos courses.</span></div>'}<ul class="plus-benefits"><li>${icon('check', 17)}Une semaine selon vos envies et votre temps</li><li>${icon('check', 17)}Des recettes qui utilisent les ingrédients du placard</li><li>${icon('check', 17)}Un plat à changer ? Les courses suivent</li><li>${icon('check', 17)}Le carnet complet à emporter ou à partager</li></ul>${plusPlans('dialog')}${billingNote()}${billingCta()}${billingFeedback()}<button class="btn-text plus-try" data-action="plus-preview">${stats.ready ? 'Retrouver ma semaine' : 'Commencer par ma semaine offerte'}</button><button class="btn-text plus-dismiss" data-action="close-dialog">Continuer gratuitement</button><p class="plus-free-note">Les ${D.foods.length} fiches sourcées, les ${D.recipes.length} recettes et votre carnet restent accessibles gratuitement.</p></div>`, { type: 'plus-offer' });
  }
  function plusPage() {
    if (Billing.active) return `${heading('Votre quotidien a le goût de Plus.', 'Vos envies, vos semaines, votre atelier.')}${billingFeedback()}${memberPanel()}${premiumNudge('home')}`;
    const examples = ['gnocchis-pesto', 'poke-saumon-cuit', 'dhal-coco'].map(id => recipesById.get(id));
    return `<div class="offer-page">${billingFeedback()}${Billing.state.canManage ? memberPanel() : ''}<section class="offer-hero"><div class="offer-hero-copy">${plusStamp()}<span class="offer-access">VOS ENVIES. VOTRE GROSSESSE. VOTRE SEMAINE.</span><h1>Le menu est prêt.<br>Vous pouvez souffler.</h1><p>Les envies changent. Les journées aussi. Préparez une semaine de plats qui vous tentent, avec ce que vous avez et le temps dont vous disposez.</p><button class="btn btn-primary offer-primary" data-action="plus-preview">Composer ma semaine offerte ${icon('arrow', 18)}</button><small>Votre première semaine avec un compte gratuit, sans carte bancaire. Les courses et le carnet sont inclus.</small><button class="btn-text offer-example-button" data-action="plus-example">Voir une semaine en exemple ${icon('arrow', 14)}</button></div><div class="offer-showcase"><span class="offer-note">Et si cette semaine ressemblait à ça ?</span>${examples.map((r, i) => `<button class="offer-dish offer-dish-${i}" data-action="recipe" data-id="${r.id}" aria-label="Découvrir la recette : ${escape(r.title)}"><img src="assets/${r.image}.jpg" alt="" width="400" height="270"><span><small>${['LUNDI · DU RÉCONFORT', 'MERCREDI · DE LA COULEUR', 'VENDREDI · UNE ESCAPADE'][i]}</small><b>${escape(r.title)}</b><em>${r.time} min · Voir la recette ${icon('arrow', 12)}</em></span></button>`).join('')}<div class="offer-shopping-note">${icon('bag', 19)}<span>Les plats choisis.<br><b>Les courses rassemblées.</b></span>${icon('check', 19)}</div><p class="offer-example-note">Exemple de recettes du catalogue. Votre semaine sera composée selon vos choix.</p></div></section><div class="offer-value-strip"><span>${icon('calendar', 17)}7 dîners ou 14 repas</span><span>${icon('fridge', 17)}Vos ingrédients en priorité</span><span>${icon('refresh', 17)}Des envies qui peuvent changer</span><span>${icon('book', 17)}Un carnet prêt à emporter</span></div><section class="offer-scenarios"><div class="section-heading"><div><span class="eyebrow">PENSÉ POUR LES VRAIES JOURNÉES</span><h2>Ce soir, vous en avez déjà assez à penser.</h2></div></div><div class="offer-scenario-grid"><article><span class="offer-scenario-icon">${icon('heart', 22)}</span><h3>« Ça ne me tente plus. »</h3><p>Écartez ce que vous n’avez pas envie de manger. Gardez un plat que vous aimez, remplacez le suivant.</p><a href="#atelier" class="btn-text">Choisir mes envies ${icon('arrow', 14)}</a></article><article><span class="offer-scenario-icon">${icon('fridge', 22)}</span><h3>« J’ai déjà tout ça au frigo. »</h3><p>Courgettes, riz, pois chiches… L’atelier privilégie les recettes qui utilisent vos ingrédients.</p><a href="#atelier" class="btn-text">Partir de mon placard ${icon('arrow', 14)}</a></article><article><span class="offer-scenario-icon">${icon('users', 22)}</span><h3>« Tu peux t’occuper du dîner ? »</h3><p>Emportez un carnet avec le menu, les courses et toutes les étapes, pour la personne qui cuisine avec vous.</p><a href="#atelier" class="btn-text">Préparer notre semaine ${icon('arrow', 14)}</a></article></div></section><section class="offer-how"><div><span class="eyebrow">DE L’ENVIE À L’ASSIETTE</span><h2>Une semaine que vous pouvez<br>vraiment faire à votre façon.</h2><ol><li><b>Vous donnez le ton.</b><span>Réconfort, couleurs ou saveurs d’ailleurs. Votre temps, vos préférences et les ingrédients disponibles.</span></li><li><b>Vous ajustez jusqu’à avoir envie.</b><span>Épinglez vos coups de cœur et changez un plat. Les quantités de la liste de courses se recalculent.</span></li><li><b>Vous emportez le tout.</b><span>Ajoutez les repas à votre carnet. Cochez ce que vous avez déjà, puis récupérez le document pour cuisiner.</span></li></ol><button class="btn btn-primary" data-action="plus-preview">Essayer avec mes envies ${icon('arrow', 16)}</button></div><img src="assets/brand/pregnancy-notebook.webp" alt="Une femme enceinte prend un moment pour préparer son carnet." width="700" height="700"></section><section class="offer-pricing-section" aria-labelledby="offer-price-title"><div class="offer-pricing-copy"><span class="eyebrow">LES REPÈRES RESTENT GRATUITS</span><h2 id="offer-price-title">Plus de place<br>pour votre quotidien.</h2><p>L’offre Plus porte sur l’organisation complète de vos repas. Le guide, les sources et les recettes restent ouverts à toutes.</p><ul class="plus-benefits"><li>${icon('check', 17)}${D.foods.length} aliments expliqués et sourcés, gratuits</li><li>${icon('check', 17)}${D.recipes.length} recettes et leurs précautions, gratuites</li><li>${icon('check', 17)}Vos favoris, menus manuels et courses, gratuits</li><li>${icon('check', 17)}Vos carnets sauvegardés restent accessibles</li></ul><p class="offer-honesty">L’atelier est une aide à l’organisation. Il ne prescrit pas un régime médical et ne vérifie pas les allergies.</p></div><div class="plus-offer-panel">${plusStamp()}<h3>Vos semaines, du début à la fin.</h3><p class="plus-lead">Les envies, le placard, les menus modifiables, les courses et le carnet à partager.</p>${plusPlans('page')}${billingNote()}${billingCta()}<p class="plus-reassurance">Le pass ne se renouvelle pas. Le mensuel se résilie depuis votre espace.</p></div></section><section class="plus-faq" aria-labelledby="plus-faq-title"><h2 id="plus-faq-title">Avant de passer en cuisine.</h2><details><summary>Qu’est-ce que je peux essayer maintenant ?</summary><p>L’atelier complet : sept dîners ou quatorze repas, vos préférences, les ingrédients à utiliser, le remplacement et l’épinglage de plats, les courses regroupées et le carnet à emporter. Votre première semaine personnalisée est offerte avec un compte gratuit, sans carte bancaire. Elle reste modifiable. Plus ouvre la composition des semaines suivantes.</p></details><details><summary>Est-ce que les courses sont commandées pour moi ?</summary><p>Vous obtenez une liste avec les quantités à prévoir. Vous pouvez retirer les articles déjà à la maison, compléter votre liste dans l’app et emporter le carnet. L’atelier ne passe aucune commande et ne calcule pas un prix de panier.</p></details><details><summary>Comment fonctionne le carnet à emporter ?</summary><p>Vous téléchargez un document HTML avec les menus, les courses, les ingrédients, les étapes et les précautions. Il s’ouvre dans un navigateur, fonctionne sans connexion et propose l’impression ou l’enregistrement en PDF.</p></details><details><summary>Comment fonctionne le paiement ?</summary><p>Choisissez le mensuel ou le pass neuf mois. Après connexion à votre compte, Stripe prend en charge le paiement. Miamama active Plus après confirmation. Le mensuel se renouvelle et peut être résilié pour la prochaine échéance ; le pass est payé une seule fois.</p></details></section></div>`;
  }
  function plusPlannerBanner() {
    return `<section class="plus-planner-banner offer-planner"><div>${plusStamp()}<h2>Vos envies. Votre placard. La semaine est prête.</h2><p>Composez vos repas, changez ce qui ne vous tente pas et retrouvez toutes les courses. Votre première semaine est offerte. Plus prépare les suivantes avec vous.</p><div class="plus-banner-actions"><button class="btn btn-primary" data-action="plus-preview">Préparer ma semaine ${icon('arrow', 15)}</button><a class="btn-text" href="#plus">Découvrir l’atelier ${icon('arrow', 15)}</a></div></div><img src="assets/recipes/gnocchi.jpg" alt="" width="240" height="240"></section>`;
  }
  ...
  function guidePage() {
    const cards = [
      ...
      ['cup', 'Un œil sur ce qu’on boit.', 'Le repère est zéro alcool. Pour la caféine, Miamama retient la limite EFSA de 200 mg par jour pendant la grossesse, toutes sources additionnées. L’eau potable reste la boisson à privilégier.', 'efsa']
    ];
    return `${heading('Des repères pour être plus sereine.', 'Quelques gestes utiles à garder en tête, tout au long de la grossesse.')}${illustratedIntro('Comprendre, puis savourer.', 'Laver, cuire, conserver : des gestes concrets pour garder le plaisir de manger pendant votre grossesse.')}<div class="guide-grid">${cards.map(([symbol, title, text, source]) => `<article class="guide-card"><div class="tip-icon">${icon(symbol, 21)}</div><h2>${title}</h2><p>${text}</p>${sourceLink(source)}</article>`).join('')}</div><section class="faq-section"><h2>Les questions qui reviennent.</h2>${[
      ['Je suis immunisée contre la toxoplasmose. Est-ce que tout change ?', 'L’immunité à la toxoplasmose ne protège pas de la listériose, de la salmonellose ni des contaminants. Les repères de Miamama ne relâchent donc pas les autres précautions. Votre sage-femme ou médecin pourra préciser celles qui vous concernent.'],
      ...
      ['Un bon Nutri-Score signifie-t-il que je peux en manger ?', 'Le Nutri-Score renseigne sur la composition nutritionnelle générale. Il ne certifie ni la sécurité microbiologique, ni la cuisson, ni la compatibilité avec la grossesse. Miamama ne l’utilise pas pour autoriser un aliment.'],
      ...
      ['J’ai déjà mangé un aliment indiqué « À éviter ».', 'Cette mention ne signifie pas que vous êtes infectée. En cas d’inquiétude, de fièvre ou d’autres symptômes, contactez votre sage-femme ou médecin et précisez l’aliment consommé. Miamama ne peut pas évaluer une exposition individuelle.'],
      ['Est-ce que Miamama fonctionne sans Internet ?', 'Après une première ouverture depuis un hébergement HTTPS ou localhost, le guide, les recettes et votre carnet sont mis en cache pour une utilisation hors connexion. Une nouvelle recherche Open Food Facts nécessite Internet ; les résultats déjà consultés peuvent rester disponibles en cache pendant 24 heures.']
  ...
  function accountPanel() {
    ...
    if (state.configured === false && window.MietteRuntime?.appURL) return `<section class="account-panel">${intro}<p>La version connectée de Miamama vous permet de retrouver votre carnet sur vos autres appareils.</p><a class="btn btn-primary" href="${escape(window.MietteRuntime.appURL)}#profil">Ouvrir la version connectée ${icon('arrow', 15)}</a><p class="small muted">Vous pouvez exporter votre carnet ici, puis l’importer dans la nouvelle version.</p></section>`;
  ...
  async function submitAuth(form, data) {
    ...
    try {
      if (mode === 'forgot') {
        ...
        if (Cloud.state.user) { renderAccount(); toast('Votre compte est ouvert. Votre carnet vous suit.', 'heart'); await resumeCommerce(); }
  ...
  function renderRoute(keepDialog = false) {
    ...
    syncSidebar();
    document.title = `${labels[route.page]} — Miamama`;
  ...
  function foodDetail(id) {
    ...
    const top = `<div class="dialog-food-top"><div class="dialog-food-art">${isOFF && f.image_front_small_url ? `<img src="${escape(f.image_front_small_url)}" alt="${escape(f.name)}" referrerpolicy="no-referrer">` : art(f.art)}</div><div class="dialog-food-heading"><span class="eyebrow">${isOFF ? 'PRODUIT OPEN FOOD FACTS' : 'LE GUIDE MIAMAMA'}</span><h2 id="dialog-title">${escape(f.name)}</h2>${badge(f.status)}<p>${escape(isOFF ? [f.brands, f.quantity].filter(Boolean).join(' · ') : f.summary)}</p></div></div>`;
    let body;
    if (isOFF) {
      const a = f.analysis;
  ...
  document.addEventListener('click', event => {
    ...
    switch (action) {
      ...
      case 'download-shopping': if (!store.shopping.length) { toast('Ajoutez quelques articles avant d’exporter.', 'info'); break; } downloadFile('miamama-mes-courses.txt', 'MIAMAMA — MA LISTE DE COURSES\n\n' + store.shopping.map(i => `[${i.checked ? 'x' : ' '}] ${i.name}${i.quantity ? ` — ${number(i.quantity)} ${i.unit}` : ''}`).join('\n'), 'text/plain;charset=utf-8'); toast('Votre liste est prête à emporter.', 'download'); break;
      ...
      case 'export-data': downloadFile('miamama-mon-carnet.json', JSON.stringify({ application: 'Miamama', version: 1, exportedAt: new Date().toISOString(), notebook: store }, null, 2), 'application/json'); toast('Votre carnet a été exporté.', 'download'); break;
