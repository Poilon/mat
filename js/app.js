(() => {
  ...
  function sidebar() {
    ...
    return `<button class="icon-button sidebar-close" data-action="close-menu" aria-label="Fermer le menu">${icon('close')}</button>
      <a class="brand" href="#accueil" aria-label="Miamama, accueil"><img src="assets/brand/mark.svg" alt="" width="32" height="32"><span class="wordmark">miam<span>ama</span></span></a><p class="brand-tagline">À table, pendant la grossesse.</p>
      <nav aria-label="Navigation principale"><p class="nav-label">Pour commencer</p>${links.map(nav).join('')}<p class="nav-label secondary">Mon carnet</p>${notebook.map(nav).join('')}<p class="nav-label secondary">À lire</p>${nav(['guide', 'book', 'Les précautions'])}${nav(['sources', 'shield', 'Sources & méthode'])}</nav>
      <div class="sidebar-bottom"><a class="sidebar-plus" href="#plus"><span>${Billing.active ? 'Mon abonnement' : 'Les menus avec Plus'}</span>${icon('arrow', 16)}</a><p>Vos recettes et votre carnet<br>restent accessibles gratuitement.</p></div>`;
  }
  function topbar() {
    return `<button class="mobile-menu" data-action="menu" aria-label="Ouvrir le menu" aria-expanded="false" aria-controls="sidebar">${icon('menu', 22)}</button><div class="breadcrumb"><a href="#accueil">Miamama</a><span aria-hidden="true">/</span><b>${labels[route.page]}</b></div><div class="topbar-right"><a href="#plus" class="topbar-plus">${Billing.active ? 'Mon accès Plus' : 'Miamama Plus'}</a><a href="#profil" class="profile-trigger" aria-label="Mon compte et mes préférences"><span class="avatar">${store.name ? escape(store.name.charAt(0).toUpperCase()) : icon('user', 17)}</span><span>${escape(store.name || (Cloud.state.user ? 'Mon compte' : 'Se connecter'))}</span></a></div>`;
  }
  function footer() {
    return `<footer class="footer"><span><span class="footer-brand">miamama</span> · Alimentation & grossesse</span><div class="footer-links"><a href="#sources">Sources & méthode</a><a href="#confidentialite">Vos données</a><a href="#plus">Plus</a></div></footer>`;
  }
  ...
  function homePage() {
    ...
    return `<div class="home-dateline"><span>${store.name ? `Bonjour ${escape(store.name)}.` : 'Bonjour.'}</span><time datetime="${localDate()}">${date}</time></div>
      <section class="home-editorial" aria-labelledby="home-title"><div class="home-search-panel"><span class="section-kicker">Alimentation & grossesse</span><h1 id="home-title">Un doute sur<br>un aliment ?</h1><p>Du chèvre, des sushis, un café… Retrouvez les précautions à prendre et les façons de les préparer.</p>${searchForm('home-search')}<div class="suggestions"><span>Par exemple</span>${['Patate douce', 'Ananas', 'Mozzarella', 'Café'].map(q => `<a class="suggestion" href="${href('aliments', { q })}">${q}</a>`).join('')}</div><a class="home-guide-link" href="#aliments">Les ${D.foods.length} fiches du guide ${icon('arrow', 16)}</a><p class="home-source-note">Chaque fiche explique le conseil et renvoie à ses sources. <a href="#sources">Comment lire les repères</a></p></div>
      <figure class="tonight-feature"><button data-action="recipe" data-id="${tonight.id}" aria-label="Voir la recette : ${escape(tonight.title)}"><img class="home-food-photo" src="assets/${tonight.image}.jpg" alt="Un plat de lasagnes gratinées" fetchpriority="high" width="760" height="530"><span class="tonight-copy"><span class="section-kicker">Une idée pour ce soir</span><strong>${escape(tonight.title)}</strong><span>${tonight.time} min · Pour 2 personnes ${icon('arrow', 17)}</span></span></button><figcaption>Photo d’inspiration · La préparation est détaillée dans la recette.</figcaption></figure></section>
      <section class="home-foods" aria-labelledby="search-title"><div class="section-heading"><div><h2 id="search-title">Quelques aliments du quotidien</h2></div><a href="#aliments" class="btn-text">Tout le guide ${icon('arrow', 15)}</a></div><div class="food-grid">${D.foods.slice(0, 4).map(foodCard).join('')}</div></section>
      <aside class="kitchen-note"><span class="section-kicker">Côté cuisine</span><p><strong>Les fruits et légumes, même épluchés ?</strong> Lavez-les à l’eau potable avant de les éplucher ou de les couper.</p><a href="#guide" class="btn-text">Les gestes à connaître ${icon('arrow', 14)}</a></aside>
      <section class="recipes-section" aria-labelledby="recipes-title"><div class="section-heading"><div><h2 id="recipes-title">À garder pour un prochain repas</h2><p>Trois idées parmi les ${D.recipes.length} recettes.</p></div><a class="btn-text" href="#recettes">Toutes les recettes ${icon('arrow', 15)}</a></div><div class="recipe-grid">${featuredRecipes.map(recipeCard).join('')}</div></section>
      ${premiumNudge('home')}<p class="home-footer-note">Ces repères sont généraux. Votre sage-femme ou médecin peut les adapter à votre situation. <a href="#sources">En savoir plus</a></p>`;
  }
  ...
  function recipePage() {
    const s = recipeSettings();
    const featured = recipesById.get('lasagnes-epinards-ricotta');
    return `<div class="recipes-page"><section class="recipe-welcome" aria-labelledby="recipe-welcome-title"><div><span class="section-kicker">Le carnet de recettes</span><h1 id="recipe-welcome-title">On mange quoi ?</h1><p>${D.recipes.length} recettes à chercher par ingrédient, par temps de cuisine ou juste pour trouver une idée.</p><span class="recipe-welcome-note">Les précautions grossesse sont indiquées dans chaque fiche.</span></div><button class="recipe-welcome-feature" data-action="recipe" data-id="${featured.id}"><img src="assets/${featured.image}.jpg" alt="Lasagnes gratinées" width="760" height="530"><span>Les lasagnes du week-end ${icon('arrow', 16)}</span></button></section>
      ${premiumNudge('recipes')}<section class="recipe-collection-section" aria-labelledby="collections-title"><div class="section-heading"><div><h2 id="collections-title">Parcourir les collections</h2><p>Du petit-déjeuner au dîner.</p></div><button class="btn-text" data-action="recipe-collection" data-collection="all" aria-pressed="${s.collection === 'all'}">Tout explorer ${icon('arrow', 15)}</button></div><div class="recipe-collections" role="group" aria-label="Collections de recettes">${D.recipeCollections.map(c => `<button class="recipe-collection ${s.collection === c.id ? 'active' : ''}" data-action="recipe-collection" data-collection="${c.id}" aria-pressed="${s.collection === c.id}"><img src="assets/${c.image}.jpg" alt="" loading="lazy" width="180" height="120"><span><b>${c.label}</b><small>${D.recipes.filter(r => r.collection === c.id).length} recettes ${icon('arrow', 12)}</small></span></button>`).join('')}</div></section>
  ...
  function plusStamp() { return `<span class="plus-stamp">miamama <b>plus</b></span>`; }
  ...
  function premiumNudge(place = 'home') {
    const copy = {
      home: ['Prévoir quelques repas à l’avance', 'Avec Plus, composez la semaine et récupérez la liste de courses.'],
      recipes: ['Des idées pour toute la semaine', 'Choisissez votre temps de cuisine et les ingrédients à utiliser.'],
      ...
      recipe: ['Trouver les autres repas de la semaine', 'L’atelier propose des menus et rassemble leurs ingrédients.']
    }[place];
    return `<aside class="premium-nudge premium-nudge-${place}" data-premium-place="${place}"><div>${plusStamp()}<h2>${copy[0]}</h2><p>${copy[1]}</p><span class="premium-nudge-caption">${Billing.active ? 'Votre atelier Plus est ouvert' : 'Première semaine offerte · Puis 4,90 €/mois ou 29,90 € pour 9 mois'}</span></div><a class="btn btn-outline" href="${Billing.active ? '#atelier' : '#plus'}">${Billing.active ? 'Ouvrir mon atelier' : 'Voir comment ça marche'} ${icon('arrow', 16)}</a></aside>`;
  }
  ...
  function plusOffer() {
    ...
    openDialog(`<div class="dialog-content plus-dialog offer-dialog">${plusStamp()}<h2 id="dialog-title">Préparer les prochaines semaines</h2><p class="plus-lead">Retrouvez des idées de repas, ajustez les plats et récupérez la liste de courses avec les recettes.</p>${stats.ready ? `<div class="offer-proof"><b>Dans votre semaine</b><div><span><strong>${stats.meals}</strong> repas</span><span><strong>${stats.matches}</strong> avec vos ingrédients</span><span><strong>${stats.items}</strong> articles à prévoir</span></div></div>` : '<div class="offer-dialog-photo"><img src="assets/recipes/gnocchi.jpg" alt="Gnocchis dorés, une des recettes à composer dans l’atelier." width="600" height="220"><span>Un exemple de plat proposé dans l’atelier.</span></div>'}<ul class="plus-benefits"><li>${icon('check', 17)}7 dîners ou 14 repas selon vos préférences</li><li>${icon('check', 17)}Des recettes qui utilisent les ingrédients du placard</li><li>${icon('check', 17)}Remplacement des plats et quantités recalculées</li><li>${icon('check', 17)}Le carnet avec les recettes et les courses</li></ul>${plusPlans('dialog')}${billingNote()}${billingCta()}${billingFeedback()}<button class="btn-text plus-try" data-action="plus-preview">${stats.ready ? 'Retrouver ma semaine' : 'Commencer par ma semaine offerte'}</button><button class="btn-text plus-dismiss" data-action="close-dialog">Continuer gratuitement</button><p class="plus-free-note">Les ${D.foods.length} fiches sourcées, les ${D.recipes.length} recettes et votre carnet restent accessibles gratuitement.</p></div>`, { type: 'plus-offer' });
  }
  function plusPage() {
    ...
    return `<div class="offer-page">${billingFeedback()}${Billing.state.canManage ? memberPanel() : ''}
      <section class="offer-hero"><div class="offer-hero-copy">${plusStamp()}<h1>Qu’est-ce qu’on mange<br>cette semaine ?</h1><p>Les jours où les idées manquent, l’atelier propose des repas en tenant compte de votre temps et de ce qu’il reste dans le placard.</p><p>Vous pouvez changer un plat, garder ceux qui vous plaisent et emporter les courses avec les recettes.</p><button class="btn btn-primary offer-primary" data-action="plus-preview">Essayer ma semaine offerte ${icon('arrow', 17)}</button><small>Avec un compte gratuit, sans carte bancaire.</small><button class="btn-text offer-example-button" data-action="plus-example">D’abord, voir un exemple ${icon('arrow', 14)}</button></div>
