(() => {
  ...
  function plusPage() {
    if (Billing.active) return `${heading('Votre abonnement Plus', 'Votre agenda, vos menus et vos courses. Tout pour alléger votre quotidien.')}${billingFeedback()}${memberPanel()}<div class="j-companion j-shortcuts"><a href="#calendrier">${icon('calendar',20)}Mon calendrier et ses exports</a><a href="#atelier">${icon('recipe',20)}Composer mes repas</a></div>${premiumNudge('home')}`;
  ...
  function renderRoute(keepDialog = false) {
    ...
    syncSidebar();
    Tonight.enter();
    document.title = route.page === 'accueil' ? 'Poum — Ma grossesse, une étape après l’autre' : `${labels[route.page]} — Poum`;
  ...
  document.addEventListener('submit', event => {
    ...
    if (form.id === 'home-search' || form.id === 'explore-search') {
      ...
      let diet; try { diet = window.PoumDiet.validate({allergies:data.getAll('allergy'),otherAllergies:String(data.get('otherAllergies')||''),avoid:String(data.get('avoid')||''),consent:data.has('diet-consent'),completed:true,temporary:store.diet.temporary}); } catch(error) {
