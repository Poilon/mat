(() => {
  ...
  function foodExplanation(f) {
    ...
    return `<section class="food-explanation" aria-labelledby="why-title"><h3 id="why-title">Pourquoi ce repère ?</h3><div class="advice-box ${f.status}"><p>${escape(e.why)}</p></div>${citations(e.sources)}<div class="evidence-mechanism"><h3>${escape(p.title)}</h3><p>${escape(p.mechanism)}</p>${citations(p.sources)}</div><div class="evidence-condition"><h3>Ce qui change la réponse</h3><p>${escape(p.condition)}</p>${e.note ? `<p class="evidence-note"><strong>Le repère en pratique.</strong> ${escape(e.note)}</p>` : ''}</div><p class="evidence-basis">${e.basis === 'specific' ? 'Synthèse Miette : cet aliment, ce type de produit ou cette préparation est cité dans les références.' : 'Application par Miette des recommandations générales à cette famille d’aliments ; les sources ne citent pas nécessairement cet aliment individuellement.'}</p></section>`;
  }
