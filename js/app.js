(() => {
  ...
  function sidebar() {
    ...
    return `<button class="icon-button sidebar-close" data-action="close-menu" aria-label="Fermer le menu">${icon('close')}</button>
      ...
      <nav aria-label="Navigation principale"><p class="nav-label">AU QUOTIDIEN</p>${links.map(nav).join('')}<p class="nav-label secondary">MON PETIT CARNET</p>${notebook.map(nav).join('')}<p class="nav-label secondary">POUR M’ACCOMPAGNER</p>${nav(['guide', 'book', 'Les bons repères'])}${nav(['plus', 'sparkle', 'Découvrir l’offre Plus'])}</nav>
  ...
  function plusStamp() { return `<span class="plus-stamp">${icon('sparkle', 14)}l’atelier <b>plus</b></span>`; }
