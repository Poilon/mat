# Miamama Plus — configuration Stripe
...
Les prix, le portail et le secret du webhook sont enregistrés dans l’environnement **Production** du projet Vercel `mat`. La clé `STRIPE_SECRET_KEY` de production reste à ajouter depuis le Dashboard Stripe dans Vercel. Le webhook reste désactivé jusqu’à la vérification du déploiement. Tant que la configuration est incomplète, aucun checkout n’est proposé et l’atelier reste offert.

Après ajout de la clé serveur :

1. Redéployer `mat` avec les variables Production. `GET /api/billing` doit indiquer `configured: true` et `mode: live` ; sans session, aucun droit n’est renvoyé.
2. Vérifier la signature sur `https://mat-sandy-six.vercel.app/api/stripe-webhook` avec le secret déjà enregistré, puis activer **uniquement** `we_1UEPYhBKxdkJ2tu1uWpDyVog` dans Stripe.
3. Vérifier la création et l’expiration d’un Checkout, sans effectuer de paiement réel de test. Suivre les livraisons d’événements dans Stripe.
