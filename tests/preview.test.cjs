...
const user={id:'owner',email:'poilon@gmail.com',emailVerified:true};
...
test('Preview requires the authenticated verified owner and ignores preferences for other accounts',()=>{
 const {req,headers}=selected('plus');assert.deepEqual(P.preview(req,user),{eligible:true,mode:'plus'});
 for(const other of [null,{...user,id:'another'}, {...user,email:'someone@example.com'}, {...user,emailVerified:false},{...user,emailVerified:'true'}]){
...
test('Recipe authorization follows both preview modes without changing another account',async()=>{
 ...
 await assert.rejects(readRecipe(id,{config,user:{...user,emailVerified:false},repo:P.effectiveRepo(selected('plus').req,{...user,emailVerified:false},repo)}),e=>e.status===402);
});
