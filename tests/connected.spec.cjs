...
test('Registration validates passwords, recovery and reset remain usable',async({page})=>{
  ...
  await page.locator('#auth-confirm').fill('password-test-123');await page.locator('#auth-form [type="submit"]').click();await expect(page).toHaveURL(/#bienvenue$/);await expect(page.locator('#ob-title')).toContainText('Enceinte.');await page.goto('/#profil');await expect(page.locator('.account-email')).toBeVisible();
...
test('Legacy notebooks import with confirmation and Poum exports round-trip on mobile',async({page})=>{
  ...
  await expect(page.locator('.install-panel')).toContainText('Poum');
  ...
  expect(download.suggestedFilename()).toBe('poum-mon-carnet.json');
  ...
  expect(exported.application).toBe('Poum');expect(exported.notebook.favorites).toEqual(['recipe:sunny-bowl']);
  await page.locator('#notebook-file').setInputFiles({name:'poum-mon-carnet.json',mimeType:'application/json',buffer:contents});
  ...
  await page.goto('/#confidentialite');
  await page.locator('#notebook-file').setInputFiles({name:'miamama-mon-carnet.json',mimeType:'application/json',buffer:Buffer.from(JSON.stringify({...exported,application:'Miamama'}))});
  await expect(page.locator('dialog')).toContainText('Importer ce carnet');await page.locator('[data-action="confirm-import"]').click();
  await page.goto('/#favoris');await expect(page.locator('.recipe-card')).toHaveCount(1);
});
...
test('Allergies synchronize to a second device, remain isolated by account, and can be withdrawn',async({page,browser})=>{
 ...
 await page.locator('[name="allergy"][value="milk"]').check();await page.locator('[name="diet-consent"]').check();await page.locator('#profile-form [type="submit"]').click();await page.goto('/#envies');await page.locator('#craving-avoid').fill('coriandre');await page.locator('#journey-cravings [type=submit]').click();await page.evaluate(()=>MietteCloud.sync());expect(remote[user.id].notebook.diet.allergies).toEqual(['milk']);await page.goto('/#profil');
 const context=await browser.newContext();try{const second=await context.newPage();await mockAccount(second,remote,user);await second.goto('http://localhost:4176/#profil');await expect(second.locator('[name="allergy"][value="milk"]')).toBeChecked();await second.goto('http://localhost:4176/#envies');await expect(second.locator('#craving-avoid')).toHaveValue('coriandre');await second.goto('http://localhost:4176/#profil');
 ...
 const other=await context.newPage();await mockAccount(other,remote,{...user,id:'account-b',email:'b@example.com'});await other.goto('http://localhost:4176/#profil');await expect(other.locator('[name="diet-consent"]')).not.toBeChecked();await other.goto('http://localhost:4176/#envies');await expect(other.locator('#craving-avoid')).toHaveValue('');
