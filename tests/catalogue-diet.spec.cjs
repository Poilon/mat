...
test('Unavailable browser storage shows a persistent explanation instead of claiming the allergies are saved',async({page})=>{
 await page.goto('/#profil');
 await page.evaluate(()=>{Storage.prototype.setItem=function(){throw new DOMException('Storage unavailable','QuotaExceededError');};});
 await page.locator('#profile-allergy-milk').check();await page.locator('#profile-consent').check();await page.locator('#profile-save').click();
 await expect(page.locator('#profile-feedback')).toContainText('votre navigateur ne peut pas les enregistrer');
 await expect(page.locator('#profile-allergy-milk')).toBeChecked();
});
