...
test('Sitemap includes only canonical public pages; mirror has noindex and the home content exists before JavaScript',()=>{
 ...
 try{for(const mirror of [false,true]){const dir=path.join(root,mirror?'mirror':'live');fs.mkdirSync(dir);fs.copyFileSync('index.html',path.join(dir,'index.html'));const result=SEO.writeSEO(dir,{mirror});const html=fs.readFileSync(path.join(dir,'index.html'),'utf8');assert.match(html,/<h1>Votre grossesse, une étape après l’autre\.<\/h1>/);assert.match(html,/Mozzarella/);assert.match(html,/<div id="app"><div class="seo-fallback">/);assert.ok(!html.includes('initial-loader'));assert.ok(graphs(html).some(n=>n['@type']==='WebSite'&&n.name==='Poum'));
