...
Avant de changer le domaine de production, l’ajouter aux domaines autorisés de Neon Auth. Les domaines actuellement autorisés sont `https://mat-sandy-six.vercel.app` et `https://poum.app`, avec le nom d’application « Poum ». La configuration actuelle se trouve dans `neon_auth.project_config` ; le domaine a été ajouté explicitement à `trusted_origins`. Le développement local reste autorisé. Ne pas autoriser tous les domaines ni modifier le DNS racine de `poilon.com` pour cette application.

`npm run db:migrate` initialise les tables du carnet, de facturation, de semaine offerte et de quotas. [La configuration Stripe est documentée séparément](STRIPE.md). Neon crée et gère son schéma d’authentification. Le retrait d’un compte passe par une réauthentification auprès de Neon Auth, puis une transaction qui supprime son carnet, les vérifications associées et son utilisateur ; les clés étrangères Neon suppriment ses sessions et identifiants de connexion. Ce chemin dépend du schéma Better Auth et doit être revérifié après une évolution du service.

## Données et réseau

La marque est devenue **Poum**. Les anciens identifiants internes `Miette*`, les clés locales `miette-*`, le protocole de transfert et les tables existantes sont conservés pour retrouver les carnets sans migration. Les exports portent désormais le nom Poum ; l’import accepte aussi les anciens fichiers Miamama, Nidelle et Miette. Le dépôt et les adresses de déploiement restent ceux du projet `mat`.
...
- Le relais Open Food Facts autorise seulement la recherche et la lecture de produits. Cache serveur et CDN, délais maximaux, quotas partagés en base et identification `User-Agent` de Poum. Recherche uniquement à la validation, jamais à chaque frappe.
...
Chaque fiche distingue une recommandation citant l’aliment ou sa préparation d’une application par Poum des conseils généraux à sa famille. Les sources ne sont pas présentées comme une validation individuelle de chacun des 360 aliments.
...
js/plus.js             Préférences, brouillons, prix et regroupement des courses
js/billing.js          Statut serveur, Checkout et portail client
server/planner.cjs     Composition des semaines, exécutée uniquement côté serveur
server/billing.cjs     Stripe, factures payées, pass et événements signés
...
api/                   Fonctions Vercel : compte, carnet, produits, atelier et Stripe
...
Les données OFF sont sous [Open Database License](https://opendatacommons.org/licenses/odbl/1-0/), les contenus individuels sous Database Contents License et les images produits sous CC BY-SA. Chaque fiche produit renvoie à sa source. Cette base est distincte du guide éditorial Poum.

Photos d’inspiration : Unsplash, voir `assets/CREDITS.md`. Les images ne remplacent pas les ingrédients écrits des recettes. Illustrations SVG créées pour Poum. Les trois anciennes scènes de grossesse ont été générées avec imagegen. Elles sont archivées dans `assets/brand/`, avec leur provenance, et ne sont plus affichées ni précachées. DM Sans et Lora sont distribuées sous SIL Open Font License ; les licences sont incluses dans `assets/fonts/`.
