...
const Preview = require('../server/preview.cjs');
module.exports = handler(async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    return json(res, 200, Preview.status(req, session?.user, await service().status(session?.user)));
  }
  ...
  if (body?.action === 'preview') {
    const selected = Preview.select(req, res, user, body.mode);
    return json(res, 200, Preview.status(selected, user, await billing.status(user)));
  }
  Preview.requireRealMode(req, user);
