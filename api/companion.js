...
module.exports=handler(async(req,res)=>{
  ...
  if(!(await require('../server/preview.cjs').effectiveRepo(req,user,billingStore()).access(user.id,config.live)).active)throw new HttpError(402,'L’export vers votre agenda fait partie de Poum Plus. Votre calendrier reste accessible gratuitement dans Poum.');
