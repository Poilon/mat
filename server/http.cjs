...
function allowRequest(req, res, { publicRead = false } = {}) {
  ...
  if (publicRead && origin === 'https://poilon.com') {
    ...
    throw new HttpError(403, 'Cette connexion ne vient pas de l’application Miette.');
  }
  if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method) && origin !== own) throw new HttpError(403, 'Rechargez Miette avant de réessayer.');
}
...
function handler(fn, options) {
  return async (req, res) => {
    try { allowRequest(req, res, options); await fn(req, res); }
      ...
      if (!error.status) console.error('Miette service error:', error.name, error.code || 'upstream');
    }
  };
}
