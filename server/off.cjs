...
async function fetchProducts({ term, code, barcode, page }, { fetchImpl = fetch, allowance } = {}) {
  ...
  for (let attempt = 0; attempt < hosts.length; attempt++) {
    ...
    try {
      const response = await fetchImpl(url, { headers: { 'User-Agent': 'Miamama/2.2.1 (https://github.com/Poilon/mat)', Accept: 'application/json' }, signal: AbortSignal.timeout(6500) });
